...
def get_normalizer(f_train,
                   mask_train=None,
                   normalizer_type="Range"):

    if normalizer_type == "Gauss":
        normalizer = UnitGaussianNormalizer(dim=tuple(range(f_train.ndim - 1)))
        normalizer.fit(f_train)
    elif normalizer_type == "Range":
        normalizer = RangeNormalizer(min=0.0, max=1.0)
...
class RangeNormalizer(Transform):
    """
    RangeNormalizer normalizes data to a given range [min, max].
    It only uses the valid data points defined by the mask when computing min/max.
    """

    def __init__(self, min=-1.0, max=1.0):
        super().__init__()

        self.register_buffer("min_data", None)
        self.register_buffer("max_data", None)
    ...
    def fit(self, x, mask=None):
        """
        Computes min and max from the data x, considering mask.
        x: (ndata, max_nnodes, d)
        mask: (ndata, max_nnodes, 1)
        """

        d = x.shape[-1]

        self.min_data = torch.zeros(x.shape[-1], device=x.device, dtype=x.dtype)
        self.max_data = torch.zeros(x.shape[-1], device=x.device, dtype=x.dtype)
        ...
        self.min_data = x_valid.min(dim=0).values
        self.max_data = x_valid.max(dim=0).values
            

    def transform(self, x):
        """
        Normalize x to target range.
        """

        shape = [1] * (x.ndim - 1) + [-1]

        min_data = self.min_data.view(*shape)
        max_data = self.max_data.view(*shape)

        x_norm = self.min + (self.max - self.min) * (x - min_data) / (max_data - min_data)

        return x_norm

    def inverse_transform(self, x_norm):

        shape = [1] * (x_norm.ndim - 1) + [-1]

        min_data = self.min_data.view(*shape)
        max_data = self.max_data.view(*shape)

        x = min_data + (max_data - min_data) * (x_norm - self.min) / (self.max - self.min)
    ...
    def cuda(self):
        self.min_data = self.min_data.cuda()
        self.max_data = self.max_data.cuda()
        return self

    def cpu(self):
        self.min_data = self.min_data.cpu()
        self.max_data = self.max_data.cpu()
        return self

    def to(self, device):
        self.min_data = self.min_data.to(device)
        self.max_data = self.max_data.to(device)
        return self
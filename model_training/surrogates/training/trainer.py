...
import surrogates.mpu.comm as comm
...
class Trainer:
    ...
    def evaluate(self, loss_dict, data_loader, log_prefix="", epoch=None, mode="single_step", max_steps=None):
        ...
        with torch.no_grad():
            for idx, sample in enumerate(data_loader):
                ...
                should_plot = True if idx==0 else False
                if idx == len(data_loader) - 1:
                    return_output = True
                if mode == "single_step":
                    eval_step_losses, outs = self.eval_one_batch(sample, loss_dict, return_output=return_output, should_plot=should_plot)
    ...
    def eval_one_batch(self,
        ...
        if should_plot and self.plot is not False:
            self.plot(out, sample, eval_step_losses, save_dir=self.save_dir)
    ...
    def log_training(self, 
        ...
        msg += f"{self.model.info()}, " if hasattr(self.model, "info") and callable(getattr(self.model, "info")) else ""
        msg = msg[:-2]
        msg += f"\n  Train: avg_loss={avg_loss:.8f}, "
        msg += f"train_err={train_err:.8f}"
        if avg_lasso_loss is not None:
            msg += f", avg_lasso={avg_lasso_loss:.8f}"
